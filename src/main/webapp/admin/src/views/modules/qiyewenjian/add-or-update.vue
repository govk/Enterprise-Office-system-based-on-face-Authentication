	...
	<div class="addEdit-block" :style='{"padding":"30px"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="文件分类" prop="wenjianfenlei">
					<el-select :disabled="ro.wenjianfenlei" v-model="ruleForm.wenjianfenlei" placeholder="请选择文件分类" >
						<el-option
							v-for="(item,index) in wenjianfenleiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="文件分类" prop="wenjianfenlei">
					<el-input v-model="ruleForm.wenjianfenlei"
						placeholder="文件分类" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="相关部门" prop="xiangguanbumen">
					<el-select :disabled="ro.xiangguanbumen" v-model="ruleForm.xiangguanbumen" placeholder="请选择相关部门" >
						<el-option
							v-for="(item,index) in xiangguanbumenOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="相关部门" prop="xiangguanbumen">
					<el-input v-model="ruleForm.xiangguanbumen"
						placeholder="相关部门" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="文件简介" prop="wenjianjianjie">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.wenjianjianjie" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.wenjianjianjie" label="文件简介" prop="wenjianjianjie">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#00a9aa","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.wenjianjianjie"></span>
                </el-form-item>
...
export default {
	data() {
		...
		return {
			...
			ro:{
				...
				wenjianfenlei : false,
				fengmian : false,
				xiangguanbumen : false,
				wenjian : false,
				gengxinshijian : false,
				wenjianjianjie : false,
			},
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='wenjianfenlei'){
							this.ruleForm.wenjianfenlei = obj[o];
							this.ro.wenjianfenlei = true;
							continue;
						}
						if(o=='fengmian'){
						...
						if(o=='xiangguanbumen'){
							this.ruleForm.xiangguanbumen = obj[o];
							this.ro.xiangguanbumen = true;
							continue;
						}
						if(o=='wenjian'){
						...
						if(o=='wenjianjianjie'){
							this.ruleForm.wenjianjianjie = obj[o];
							this.ro.wenjianjianjie = true;
							continue;
						}
				}
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.wenjianjianjie = this.ruleForm.wenjianjianjie.replace(reg,'../../../ssm4xo2n/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `qiyewenjian/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.qiyewenjianCrossAddOrUpdateFlag = false;
						 this.parent.search();
