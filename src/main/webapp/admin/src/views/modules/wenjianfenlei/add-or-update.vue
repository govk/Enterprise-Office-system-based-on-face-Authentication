<template>
	<div class="addEdit-block" :style='{"padding":"30px"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="文件分类" prop="wenjianfenlei">
					<el-input v-model="ruleForm.wenjianfenlei" placeholder="文件分类" clearable  :readonly="ro.wenjianfenlei"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="文件分类" prop="wenjianfenlei">
					<el-input v-model="ruleForm.wenjianfenlei" placeholder="文件分类" readonly></el-input>
				</el-form-item>
			</template>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `wenjianfenlei/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.wenjianfenleiCrossAddOrUpdateFlag = false;
						 this.parent.search();
